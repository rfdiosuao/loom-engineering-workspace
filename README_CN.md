...
AI 驱动的 Android 自动化应用，通过自然语言让 LLM Agent 操控 Android 设备（手机）。用户通过消息渠道（钉钉、飞书、QQ、Discord、Telegram）发送指令，AI Agent 理解意图后自主执行设备操作。项目名称现统一为 `Agent Phone`。
...
![Star History Chart](https://api.star-history.com/svg?repos=rfdiosuao/lumiapkclaw)
...
| `import_image_to_gallery` | 从 PC / Lumi 启动器接收图片，保存到手机相册 |
| `finish` | 完成任务并返回总结 |
...
| `/api/media/import_image` | POST | 接收 PC 上传图片并写入手机相册 |
