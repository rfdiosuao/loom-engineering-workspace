...
class TaskOrchestrator(
    ...
    fun startNewTask(channel: Channel, task: String, messageID: String) {
        ...
        if (TaskStartNavigationPolicy.shouldPressHomeBeforeChannelTask()) {
        ...
        agentService.executeTask(task, object : AgentCallback {
            ...
            override fun onComplete(round: Int, finalAnswer: String, totalTokens: Int) {
                ...
                // 记录行为
                BehaviorRecorder.recordTaskExecuted(task, success = true)

                // 检查是否需要生成主动建议
                checkAndPushSuggestions(channel, messageID)
     ...
     * 任务完成后，检查行为模式并生成建议
     */
    private fun checkAndPushSuggestions(channel: Channel, messageID: String) {
        try {
            // 生成建议
            val newSuggestions = ProactiveSuggestionManager.generateSuggestions()

            if (newSuggestions.isNotEmpty()) {
                // 保存建议
                newSuggestions.forEach { ProactiveSuggestionManager.addSuggestion(it) }

                // 推送第一条建议给用户
                val suggestion = newSuggestions.first()
                val message = buildSuggestionMessage(suggestion)
                ChannelManager.sendMessage(channel, message, messageID)

                XLog.i(TAG, "已推送主动建议: ${suggestion.title}")
            }
        } catch (e: Exception) {
            XLog.e(TAG, "检查主动建议失败", e)
        }
    }
     ...
     * 构建建议消息
     */
    private fun buildSuggestionMessage(suggestion: ProactiveSuggestionManager.Suggestion): String {
        return buildString {
            append("💡 AI建议\n\n")
