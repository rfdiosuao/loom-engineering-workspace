...
class KeepAliveJobService : JobService() {
    ...
    override fun onStartJob(params: JobParameters?): Boolean {
        val foregroundRunning = ForegroundService.isRunning()
        val configServerRunning = ConfigServerManager.isRunning()
        XLog.i(TAG, "KeepAlive job triggered, ForegroundService running: $foregroundRunning, ConfigServer running: $configServerRunning")
        if (KeepAliveRecoveryPolicy.shouldStartForegroundService(foregroundRunning)) {
        ...
        if (
            KeepAliveRecoveryPolicy.shouldHealConfigServer(
                configServerEnabled = KVUtils.isConfigServerEnabled(),
                llmConfigured = KVUtils.hasLlmConfig(),
                configServerRunning = ConfigServerManager.isRunning()
            )
        ) {
            ConfigServerManager.autoStartIfNeeded(applicationContext)
            XLog.i(TAG, "Healed ConfigServer, running: ${ConfigServerManager.isRunning()}")
        }
        return false
    }
