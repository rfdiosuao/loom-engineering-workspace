...
class AppPaths:
    ...
    @property
    def platform_tools_dirs(self) -> tuple[str, ...]:
        candidates: list[str] = []
        for root in self.payload_roots:
            candidates.extend([
                os.path.join(root, "platform-tools"),
                os.path.join(root, "_up_", "redist", "platform-tools"),
                os.path.join(root, "redist", "platform-tools"),
                os.path.join(root, "_up_", "platform-tools"),
                os.path.join(root, "SystemData", ".core", "platform-tools"),
            ])
        seen: set[str] = set()
        result: list[str] = []
        for path in candidates:
    ...
    def adb_exe(self) -> str:
        ...
        for directory in self.platform_tools_dirs:
            for name in names:
    ...
    @property
    def adb_is_bundled(self) -> bool:
        adb_path = self.adb_exe
        if not adb_path:
            return False
        adb_key = os.path.normcase(os.path.abspath(adb_path))
        return any(
            adb_key.startswith(os.path.normcase(os.path.abspath(directory)) + os.sep)
            for directory in self.platform_tools_dirs
        )
    ...
    def openclaw_mjs(self) -> str:
        ...
        candidates.insert(0, os.path.join(self.base_path, "data", ".installer", "npm-global", "node_modules", "openclaw", "openclaw.mjs"))
        for path in candidates:
            if os.path.exists(path):
                return path
        return candidates[0]
    ...
    def process_env(self) -> dict[str, str]:
        ...
        path_entries = [
            os.path.dirname(self.adb_exe) if self.adb_exe else "",
            self.node_dir,
            os.path.join(self.base_path, "data", ".installer", "npm-global"),
            os.path.join(self.base_path, "data", ".installer", "npm-global", "bin"),
