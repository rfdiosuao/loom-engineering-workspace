...
const releaseNotesPath = path.join(repositoryRoot, "docs", "RELEASE_NOTES_2.4.10.md");

async function readJson(relativePath) {
  return JSON.parse(await readFile(path.join(launcherRoot, relativePath), "utf8"));
}

test("Desktop 2.4.10 has one consistent version and the exact 麓鸣 product name", async () => {
  ...
  assert.equal(packageJson.version, "2.4.10");
  assert.equal(packageLock.version, "2.4.10");
  assert.equal(packageLock.packages[""].version, "2.4.10");
  assert.equal(tauriConfig.version, "2.4.10");
  assert.equal(tauriConfig.productName, "麓鸣");
  assert.equal(tauriConfig.app.windows[0].title, "麓鸣");
  assert.match(cargoToml, /^version = "2\.4\.10"$/m);
  assert.match(cargoLock, /\[\[package\]\]\s+name = "app"\s+version = "2\.4\.10"/m);
});

test("2.4.10 release notes state desktop fixes and preserve the phone boundary", async () => {
  const notes = await readFile(releaseNotesPath, "utf8");

  assert.match(notes, /麓鸣 Desktop `2\.4\.10`/);
