...
const releaseNotesPath = path.join(repositoryRoot, "docs", "RELEASE_NOTES_2.4.2.md");

async function readJson(relativePath) {
  return JSON.parse(await readFile(path.join(launcherRoot, relativePath), "utf8"));
}

test("Desktop 2.4.2 has one consistent version and the exact 麓鸣 product name", async () => {
  ...
  assert.equal(tauriConfig.version, "2.4.2");
  assert.equal(tauriConfig.productName, "麓鸣");
  assert.equal(tauriConfig.app.windows[0].title, "麓鸣");
  assert.match(cargoToml, /^version = "2\.4\.2"$/m);
  assert.match(cargoLock, /\[\[package\]\]\s+name = "app"\s+version = "2\.4\.2"/m);
});

test("2.4.2 release notes state phone version and capability limits without fake claims", async () => {
  const notes = await readFile(releaseNotesPath, "utf8");

  assert.match(notes, /麓鸣 Desktop `2\.4\.2`/);
