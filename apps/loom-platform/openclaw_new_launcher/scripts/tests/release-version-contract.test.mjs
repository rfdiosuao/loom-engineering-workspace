...
const releaseNotesPath = path.join(launcherRoot, "docs", "RELEASE_NOTES_2.4.15.md");

async function readJson(relativePath) {
  return JSON.parse(await readFile(path.join(launcherRoot, relativePath), "utf8"));
}

test("Desktop 2.4.15 has one consistent version and the exact 麓鸣 product name", async () => {
  ...
  assert.equal(packageJson.version, "2.4.15");
  assert.equal(packageLock.version, "2.4.15");
  assert.equal(packageLock.packages[""].version, "2.4.15");
  assert.equal(tauriConfig.version, "2.4.15");
  assert.equal(tauriConfig.productName, "麓鸣");
  assert.equal(tauriConfig.app.windows[0].title, "麓鸣");
  assert.match(cargoToml, /^version = "2\.4\.15"$/m);
  assert.match(cargoLock, /\[\[package\]\]\s+name = "app"\s+version = "2\.4\.15"/m);
});

test("2.4.15 release notes state the Codex sandbox and rollback fix", async () => {
  const notes = await readFile(releaseNotesPath, "utf8");

  assert.match(notes, /麓鸣 Desktop `2\.4\.15`/);
  ...
  assert.match(notes, /Windows sandbox/);
  assert.match(notes, /Provider/);
  assert.match(notes, /逐字恢复/);
