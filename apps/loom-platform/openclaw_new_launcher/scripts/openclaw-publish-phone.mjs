...
function buildPrompt(config, uploadedMediaRefs = []) {
  ...
  return [
    ...
    config.draftOnly ? '执行模式: 只保存草稿' : '执行模式: 已授权正式发布',
    '已准备素材:',
    mediaBlock,
    '',
    '执行要求:',
...
function publishBusinessFailure(answer) {
  const text = String(answer || '').trim();
  if (!text) return '';
  const failed = [
    /^\s*(?:Task completed:\s*)?任务(?:执行)?(?:受阻|阻塞)(?:\s|[：:，,。-]|$)/im,
    /任务无法完成/,
    /任务未能完成/,
    /无法完成任务/,
    /(?:发布|保存草稿)(?:失败|未完成)/,
    /(?:当前)?未登录[^\n]*(?:发布|草稿)/,
    /(?:需要|请)先登录[^\n]*(?:发布|草稿)/,
    /\b(?:cannot|unable to|failed to)\s+(?:complete|publish|save)\b/i,
    /\b(?:not logged in|login required)\b/i,
  ].some((pattern) => pattern.test(text));
  return failed ? text.slice(0, 1000) : '';
}
