...
class MenuItem @JvmOverloads constructor(
    ...
    private val switchTrailing: SwitchMaterial
    ...
    init {
        ...
        switchTrailing = findViewById(R.id.switchTrailing)
        ivTrailing = findViewById(R.id.ivTrailing)
        divider = findViewById(R.id.divider)

        switchTrailing.setOnCheckedChangeListener { _, isChecked ->
            if (!updatingSwitch) switchChangeListener?.invoke(isChecked)
        }
    ...
    fun setSwitchVisible(show: Boolean) {
        switchTrailing.isVisible = show
        if (show) {
            tvTrailing.isVisible = false
            setShowTrailingIcon(false)
        }
    }

    fun setSwitchChecked(checked: Boolean) {
        if (switchTrailing.isChecked == checked) return
        updatingSwitch = true
        switchTrailing.isChecked = checked
