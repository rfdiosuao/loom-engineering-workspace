...
Options:
  ...
  --ratio <ratio>              Common image ratio: 1:1, 3:4, 4:3, 9:16, 16:9, 5:2
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case '--ratio':
        args.ratio = next();
        if (IMAGE_RATIO_TO_SIZE[args.ratio]) {
          args.size = IMAGE_RATIO_TO_SIZE[args.ratio];
        }
        break;
      case '--count':
...
async function saveGeneratedImages(buffers, config) {
  ...
  for (let i = 0; i < buffers.length; i += 1) {
    ...
    await fs.writeFile(filePath, buffers[i], { flag: 'wx' });
    await fs.writeFile(`${filePath}.json`, JSON.stringify({
      schema: 'loom.media.asset.v1',
      prompt: config.prompt,
      mode: config.mode === 'edit' ? 'i2i' : 't2i',
      ratio: config.ratio || '',
      size: config.size || DEFAULT_SIZE,
      model: config.imageModel || DEFAULT_IMAGE_MODEL,
      source: 'cli',
...
function timestamp() {
  const wallClock = new Date().toISOString().replace(/[-:]/g, '').replace('T', '-').replace('Z', '').replace('.', '-');
  return `${wallClock}-${process.hrtime.bigint().toString(36)}`;
}
...
async function main() {
  ...
  const summary = {
    ...
    libraryDirectory: config.outDir,
    localImages,
